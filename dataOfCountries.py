...
def randomList(listcountry):
    # Generate 6-10 random numbers between 0 and length of country
    # randomlist = random.sample(range(0, len(listcountry)), random.randint(6, 10))

    # Generate 6 random numbers between 0 and length of country
    randomlist = random.sample(range(0, len(listcountry)), 6)
    return randomlist


# Short-listed the list of shops in country
...
# return a list of shop(address, longitude and latitude)
...
routeName(JPN, route)
getmap(JPN, route, "html file\\JPNmap")

print("\nUAE Shop Route")
route = FindingRoute(aeG, len(UAE.shops) - 1)
routeName(UAE, route)
getmap(UAE, route, "html file\\UAEmap")

print("\nChina Shop Route")
route = FindingRoute(cnG, len(CHN.shops) - 1)
routeName(CHN, route)
getmap(CHN, route, "html file\\CHNmap")

print("\nEngland Shop Route")
route = FindingRoute(egG, len(ENG.shops) - 1)
routeName(ENG, route)
getmap(ENG, route, "html file\\ENGmap")
